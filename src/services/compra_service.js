...
import * as produtoRepository from '../repositories/product_repository.js';
...
export async function inserir(compra) {
    ...
    const promisesItens = compra.itens.map(async (item) => {
        ...
        const produto = await produtoRepository.findProductById(item.produtoId);
        if (!produto) {
        ...
        await produtoRepository.updateProduct(produto.id, produto);
